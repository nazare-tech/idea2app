...
- `docs/SECURITY_RELEASE_CHECKLIST.md` section 1 (RLS audit for subscriptions, credits, plan_prices, stripe_credit_grants) is entirely unchecked.
...
## Confirmed Decisions
- Recommendation A chosen for question 1: keep `consume_credits` callable from authenticated user-scoped routes, but enforce `p_user_id = auth.uid()` in the RPC for non-service-role callers. Implemented 2026-06-12.
- Recommendation A chosen for question 2: re-home useful root `migrations/` SQL into dated `supabase/migrations/` files, then remove the root directory's applyable SQL. Repo-side re-home completed 2026-06-16; production object verification still depends on Supabase access.
- Recommendation A chosen for question 3: export matching legacy Stitch mockup rows to a local gitignored JSON before deleting them. Pending production Supabase access.
...
- Keep credit accounting internal; user-facing credit display removal is scheduled separately (Milestone 3 plus the existing `plans/remove-credits-and-future-entitlements-plan.md`).
...
3. [ ] Create a schema baseline: `supabase db dump --schema public` into `supabase/migrations/<ts>_baseline_schema.sql` (or `supabase db pull`), marked/ordered so existing environments treat it as already applied; prove a fresh `supabase db reset` rebuilds the schema. Attempted locally 2026-06-16, blocked because the repo is not linked/logged in (`SUPABASE_ACCESS_TOKEN` missing). Root metrics and prompt-chat SQL were re-homed separately under `supabase/migrations/20260616001000` through `20260616001400`.
4. [x] Retire the root `migrations/` directory per Clarifying Question 2 decision; the old `005_create_refund_credits.sql` must not survive anywhere applyable. Done 2026-06-16: metrics + prompt-chat SQL moved into dated Supabase migrations; old refund and duplicate mockups root SQL removed.
5. [x] Remove the Stitch code path:
...
6. [ ] Delete legacy Stitch data: count rows (`select count(*) from mockups where content like '%"type":"stitch"%'` or the equivalent JSON check), export per Clarifying Question 3, then delete those rows. Pending production Supabase access.
7. [x] Remove app generation: delete `src/app/api/generate-app/route.ts` and `src/lib/prompts/app-generation.ts`; remove the dispatch branch in `src/components/workspace/project-workspace.tsx` (around line 1166) and the `app-*` entries in `CREDIT_COSTS` (`src/lib/utils.ts`); remove `api.anthropic.com` from CSP `connect-src` if nothing else uses it; scrub App Generation and Deployment claims from `PROJECT_CONTEXT.md`. Done 2026-06-16.
8. [x] Disable general chat: add the same disabled-gate pattern as `src/app/api/prompt-chat/route.ts` (unconditional 410 with a clear message) at the top of `src/app/api/chat/route.ts`; while in the file, fix the history query to fetch the newest 20 (`ascending: false`, `limit(20)`, then reverse) so a future re-enable starts correct. Done 2026-06-16 by replacing the route with GET/POST `410 Gone`; no live history query remains.
9. [x] Dependency pass: `npm uninstall jspdf html2canvas mermaid`; `npm audit fix`; review the lockfile diff; re-run `npm audit --omit=dev` until 0 critical and 0 high. Done 2026-06-16: removed `@google/stitch-sdk` and `jspdf` (`html2canvas` was transitive; no direct `mermaid` package existed), upgraded Next to 16.2.9, aligned `eslint-config-next`, added a PostCSS override, and `npm audit --omit=dev` reports 0 vulnerabilities.
10. [ ] Run `docs/SECURITY_RELEASE_CHECKLIST.md` section 1 against production (RLS on subscriptions, credits tables, plan_prices, stripe_credit_grants); check each box with a date; write migrations for any discrepancy found. Pending production Supabase access.
11. [x] Documentation: update `PROJECT_CONTEXT.md` (mockup section drops Stitch compatibility, API list drops generate-app/stitch, chat marked disabled) and add an addendum line to `docs/SECURITY_REVIEW_2026-04-25.md` recording that finding 1 (iframe) is closed by removal. Done 2026-06-16.
