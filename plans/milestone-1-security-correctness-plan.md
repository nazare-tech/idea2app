...
10. [ ] Run `docs/SECURITY_RELEASE_CHECKLIST.md` section 1 against production (RLS on subscriptions, credits tables, plan_prices, stripe_credit_grants); check each box with a date; write migrations for any discrepancy found.
11. [ ] Documentation: update `PROJECT_CONTEXT.md` (mockup section drops Stitch compatibility, API list drops generate-app/stitch, chat marked disabled) and add an addendum line to `docs/SECURITY_REVIEW_2026-04-25.md` recording that finding 1 (iframe) is closed by removal.
