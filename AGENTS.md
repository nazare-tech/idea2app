...
- Include a Runtime and Change-Impact Analysis section in every substantial plan. For timers, polling, streams, subscriptions, retries, render loops, caches, shared state, queues, and client-server contracts, record the expected and worst-case frequency, state owner and lifetime, work per update, fan-out/blast radius, reset/invalidation and partial/duplicate/delayed/out-of-order behavior, rollback seam, and the specific measurement or test that can detect each named risk. High-frequency state should live at the lowest practical ownership boundary; placement above that boundary requires a measured reason. Do not accept generic entries such as "performance considered"—connect the mechanism to its impact and observable evidence. Use the template in `docs/plans/README.md`.
