  ...
  const sectionHtml = sections.map((section) => {
    const groups = section.optionGroups.map((group) => {
      const cards = group.cards.map((card) => {
        ...
        return `<figure><img loading="lazy" src="${escapeHtml(card.outputRelative)}" alt="${escapeHtml(label)}"><figcaption><span>Screen ${card.screenIndex}</span>${escapeHtml(card.screenName)}</figcaption></figure>`
      }).join("")
