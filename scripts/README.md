...
| `build-brand-variety-briefs.mjs` | Prepares the brand-variety validation batch: assigns each case study its deterministic kit triad and writes per-idea generation briefs. | `node scripts/build-brand-variety-briefs.mjs [--runs <dir>] [--out <dir>]` |
