...
12. **Pencil Design System**: Light-mode UI with dark sidebar; CSS custom properties for theming; Hanken Grotesk + Fira Mono remain the app defaults. A reversible `.makercompass-editorial-theme` boundary scopes Newsreader + Public Sans + JetBrains Mono and paper/ink/ultramarine semantic-token overrides to `/`, exact `/projects`, and the existing root of each valid `/projects/[projectRef]` workspace; `/projects/new` and other routes keep default tokens. Workspace document navigation, common document blocks, portal chrome, and profile surfaces use semantic tokens so the scoped palette remains coherent without changing artifact content
