...
- **Projects dashboard cards** — `/projects` loads owned projects, allowance status, and canonical mockup metadata server-side, then renders a responsive card grid with at most two cards per row, a 430px minimum track width, and 32px row and column gaps over a viewport-bounded seeded landing-page dot field owned by `DashboardShell`. The exact `/projects` route gets this decorative, pointer-inert dots-only canvas; project workspaces, intake, billing, and preferences do not. Each 500px-tall `DashboardProjectCard` matches Figma node `435:5578`: the outer shell is borderless and transparent, the white media surface has a 24px radius and 20px inset around an approximately 299px image canvas, and the transparent 160.6px details area uses 8px horizontal and 20px vertical insets. The title is 18px medium weight with hard overflow clipping; description copy flows naturally at 14px inside a hard-clipped 72px slot without CSS ellipsis, and the italic creation date stays bottom-aligned. The newest canonical mockup row supplies Version A through the authenticated image proxy; the controlled no-credit fixture may use its existing inline SVG. Absent, retired, malformed, or failed image content shows a neutral empty state, while a mockup-query failure is logged and shown as “Preview unavailable.” Cards retain responsive grid widths and focus workspace warmup. An always-visible sibling overflow button uses the existing Lucide icon, aligns its 16px-by-18px glyph with the title, and avoids nesting controls inside the full-card link; its Radix menu offers Rename and Delete. Rename is available to every plan, opens a hoisted accessible dialog, persists a normalized 1-80-code-unit name through a 15-second bounded owned `PATCH /api/projects/[id]`, and updates the UUID-plus-slug href from the returned name. Delete retains its existing behavior through accessible dialogs: Free users see an upgrade prompt, while paid users get confirmation before `DELETE /api/projects/[id]`.
