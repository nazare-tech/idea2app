...
2. **Idea Intake Wizard Flow**:
   ...
   - `POST /api/intake/questions` generates 3-5 AI-authored answer-chip questions (the prompt prefers 5) with `single` and `multiple` answer modes. Standalone text questions are rejected; the UI itself appends `Decide for me` and free-text `Other` escape hatches to every choice question except the required `primary-platform` question (see `supportsAnswerEscapeHatches` in `src/lib/intake/answers.ts`), so `allowOther` no longer gates the UI. If AI question generation is unavailable or invalid, the route returns a retryable error instead of falling back to curated questions
...
└──────────────┴───────────────────┴──────────────────────────────┘
  ...
  "Ask this project" FAB; the header hides on scroll down and returns on
  scroll up (useHideOnScrollChrome), while the peek bar stays pinned so the
  section indicator is always readable.
```
...
- **`AnchorNav`** — 300px scroll-aware document rail, rendered at `lg+` only and styled from Figma node `485:36617`: Public Sans 17px document headings with matching Lucide icons, 14px subsection rows on a 34px rhythm, CSS branch connectors, and a dark active subsection. Completed documents stay visually quiet to preserve the designed hierarchy; exceptional `Queued`, `Generating`, `Generate`, `Needs retry`, and `Retry` states remain available from `DocumentGenerationDisplayState`. Status/action policy primitives live in `src/components/layout/nav-status.tsx`, shared with the mobile bottom chrome, while the mobile navigation itself remains unchanged.
- **`MobileDocumentBar`** (`src/components/workspace/mobile-document-bar.tsx`) — below `lg`: a curved-top peek bar naming the document in view (status marker + label) that opens a bottom sheet listing every document with its subsections and generation status; taps route through the same `handleScrollNavigate` path as the rail. The peek bar is always on screen (only the slim header hides on scroll, via `use-hide-on-scroll-chrome.ts`), so the section indicator never scrolls away. The "Ask this project" composer collapses to a primary-color FAB that rides above the peek bar or the open sheet, and expands into its own bottom sheet sized by `--workspace-composer-sheet-max-height`. Sheets and bottom chrome pad with `env(safe-area-inset-bottom)` (root layout sets `viewport-fit=cover`, shells use `h-dvh`). Keyboard avoidance is two-part: the root layout also sets `interactive-widget=resizes-content` for Chrome Android, and `use-keyboard-inset.ts` publishes `--workspace-keyboard-inset` from the visual viewport for iOS Safari, which the composer sheet uses as its bottom offset and height clamp.
...
- **Projects dashboard cards** — `/projects` loads owned projects, allowance status, and canonical mockup metadata server-side, then renders a responsive card grid with at most two cards per row, a 430px minimum track width, and 32px row and column gaps on the standard dashboard background. Each 500px-tall `DashboardProjectCard` matches Figma node `435:5578`: the outer shell is borderless and transparent, the 378px white media surface has a 1px border, 24px radius, and 20px inset around a 336px image canvas, and the transparent 122px details area uses 8px horizontal and 20px top padding. Its 22px heading row places an 18px medium, single-line ellipsized title beside the 14px italic creation date; 14px description copy flows naturally inside a hard-clipped 72px slot without CSS line clamping or ellipsis. The newest canonical mockup row supplies valid options in A/B/C order through the authenticated image proxy; the controlled no-credit fixture may use exact inline SVGs for all three options. Absent, retired, malformed, or failed image content shows a neutral empty state, while a mockup-query failure is logged and shown as “Preview unavailable.” Multi-option cards start on A and expose persistent position dots, roving dot keyboard focus, and announced slide changes. Switching to an uncached option by arrow, dot, keyboard, or swipe places a centered, non-interactive loading spinner over the media canvas until that exact image loads or fails; cached options display immediately. Desktop hover/focus reveals the top-right vertical kebab and only the available 32px previous/next media arrows; coarse no-hover devices keep the kebab visible, visually suppress arrows, and switch slides by horizontal swipe on the media canvas. Chromium's native gesture handling prevents a qualifying swipe from activating the card link, while the next genuine tap opens the project. All carousel controls remain siblings of the full-card link. The kebab's Radix menu offers Rename, Export Project, and Delete. Export fetches an authenticated owner-scoped ZIP assembled in memory from the latest canonical visible artifacts; archive paths are server-derived, mockup bytes come only from validated canonical Storage paths, unsupported/oversized/missing images become manifest warnings, and no new ZIP dependency is required. Rename is available to every plan, opens a hoisted accessible dialog, persists a normalized 1-80-code-unit name through a 15-second bounded owned `PATCH /api/projects/[id]`, and updates the UUID-plus-slug href from the returned name. Delete retains its existing behavior through accessible dialogs: Free users see an upgrade prompt, while paid users get confirmation before `DELETE /api/projects/[id]`.
- **Project export runtime** — Export input bytes remain bounded in memory, but ZIP records stream through the HTTP response to avoid Vercel's buffered 4.5 MB limit. Independently keyed user/IP rate limits protect egress, and service-role mockup reads require exact finalized-run metadata rather than trusting mutable row content alone.
...
10. **Shared UI Registries + Hooks**: Repeated view metadata and repeated client behaviors (documents, billing portal, auth sign-out, chat interactions) are centralized into typed registries and reusable hooks/components before page-level assembly. Every HTML table visible in generated project reports composes through `ReportTable`, which owns the shared responsive shell and visual treatment while document renderers retain semantic rows, links, widths, and optional emphasis markers
11. **Path Aliases**: Clean imports using `@/*` aliases
12. **Pencil Design System**: Light-mode UI with dark sidebar; CSS custom properties for theming; Hanken Grotesk + Fira Mono remain the app defaults. A reversible `.makercompass-editorial-type` boundary scopes Newsreader + Public Sans + JetBrains Mono to `/`, exact `/projects`, and the existing root of each valid `/projects/[projectRef]` workspace. These pages inherit the established Warm Horizon color tokens, default Action Red mark, and red primary actions; `/projects/new` and other routes keep the default type families. Workspace document navigation, common document blocks, portal chrome, and profile surfaces use semantic color tokens without changing artifact content
