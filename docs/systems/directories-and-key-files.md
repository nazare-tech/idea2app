...
| [src/components/landing/hero-dot-field.tsx](src/components/landing/hero-dot-field.tsx) | Ambient section-background canvas: deterministic clustered short-line lattice mounted inside each selected landing section, so native document/sticky layout moves or pins it with its content; cursor magnetic rings, tangential line alignment, event-driven animation parking, lifecycle gating, and pointer-exit reset |
