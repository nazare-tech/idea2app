...
| [src/components/layout/dashboard-shell.tsx](src/components/layout/dashboard-shell.tsx) | Authenticated dashboard shell for top-level dashboard, projects, billing, and preferences pages; owns the exact-route, viewport-bounded dots-only `/projects` background |
| [src/app/(dashboard)/projects/page.tsx](src/app/(dashboard)/projects/page.tsx) | Projects dashboard — loads owned projects, allowance status, and Version A mockup thumbnail metadata, then renders cards with project action menus and paid-plan delete gating |
| [src/components/projects/dashboard-project-card.tsx](src/components/projects/dashboard-project-card.tsx) | Interactive project card with a top mockup preview, workspace warmup, title-row Rename/Delete menu, accessible rename dialog, delete confirmation, and Free-plan upgrade prompt |
| [src/components/projects/dashboard-project-card-details.tsx](src/components/projects/dashboard-project-card-details.tsx) | Figma-matched transparent details area with title action-space reservation, natural hard-clipped description flow, and bottom-aligned creation date |
| [src/components/projects/dashboard-project-thumbnail.tsx](src/components/projects/dashboard-project-thumbnail.tsx) | Rounded white project-card media surface with lazy authenticated image loading plus distinct missing/failed-image and query-unavailable states |
| [src/lib/mockups/dashboard-thumbnail.ts](src/lib/mockups/dashboard-thumbnail.ts) | RLS-bound dashboard mockup-row reduction, canonical Version A proxy URL derivation, and exact no-credit fixture handling |
...
| [src/app/page.tsx](src/app/page.tsx) | Landing page with dynamic signup vs waitlist CTA rendering, authenticated-user redirect, section-local cursor-reactive dot-field backdrops plus hero build map, and the scroll-driven features section |
| [src/components/landing/hero-build-map.tsx](src/components/landing/hero-build-map.tsx) | Active hero artwork: five artifact nodes (Idea, Research, Plan, Design, Prompt) wired by drawn SVG connectors, rendered as a wide and a tall canvas swapped at 1024px, cycling through three sample ideas from one effect |
| [src/components/landing/hero-dot-field.tsx](src/components/landing/hero-dot-field.tsx) | Reusable ambient section-background canvas: deterministic clustered dot lattice for landing sections and the projects dashboard; cursor-proximity dot lift, optional compass wedges, event-driven animation parking, lifecycle gating, and pointer-exit reset |
| [src/components/landing/hero-dot-field-core.ts](src/components/landing/hero-dot-field-core.ts) | Pure, unit-tested dot-field logic: seeded lattice/cluster layout, wedge-site selection, shortest-arc angle damping, protected-zone alpha falloffs, and the single shouldAnimate lifecycle gate |
| [src/lib/compass-geometry.ts](src/lib/compass-geometry.ts) | Shared brand bearing-wedge geometry (1024-viewBox points, SVG path builder, canvas tracer) consumed by both `compass-mark.tsx` and the hero dot field so the mark cannot drift between renderers |
| [src/lib/landing-hero-build-map.ts](src/lib/landing-hero-build-map.ts) | Both build-map canvases (node, joint, and connector geometry in canvas percentages), the three cycling scenarios, and the cycle's timing constants |
| [src/components/landing/hero-artwork.tsx](src/components/landing/hero-artwork.tsx) | Inactive, retained implementation of the former desktop-gated layered sticky-note artwork, kept with its `public/landing/hero/` assets for possible restoration |
| [src/components/landing/hero-reel-arc.tsx](src/components/landing/hero-reel-arc.tsx) | Inactive, retained implementation of the former hero reel: ten normalized transparent mobile-device cutouts repeated around the crown of a 3000px circle, with `public/landing/hero-reel/` assets and its `.hero-reel-*` CSS kept for possible restoration |
...
| [src/lib/mockups/design-plan.ts](src/lib/mockups/design-plan.ts) | Hidden mockup design-plan prompt plus strict parsing for platform, two-screen happy path, A/B/C directions, and optional capped persisted style selections. |
| [src/lib/mockups/openrouter-image-pipeline.ts](src/lib/mockups/openrouter-image-pipeline.ts) | Storyboard generation, Pro Max/legacy whole-triad resolution, prompt and CTA-semantic formatting, rollback flags, image config, dimension validation, and Storage upload. |
| [src/lib/mockups/pro-max-style-selector.ts](src/lib/mockups/pro-max-style-selector.ts) | Product-aware deterministic Foundation / Distinctive / Experimental selector, controlled-transport verification, prompt formatter, fallback diagnostics, and platform-safe style rules. |
| [src/lib/mockups/pro-max-style-types.ts](src/lib/mockups/pro-max-style-types.ts) | Shared Pro Max treatment, triad, persistence, field-cap, and prompt-cap contracts. |
| [src/lib/mockups/pro-max-style-catalog.generated.ts](src/lib/mockups/pro-max-style-catalog.generated.ts) | Generated sub-100 KB runtime catalog compiled from frozen pinned Pro Max design-system results; regenerate through `scripts/generate-pro-max-style-catalog.mjs`. |
| [vendor/ui-ux-pro-max/product-design-systems.json](vendor/ui-ux-pro-max/product-design-systems.json) | Frozen raw output from the pinned skill's `search.py --design-system --json` workflow for every upstream product category; author-time only and excluded from runtime imports. |
| [.agents/skills/ui-ux-pro-max/](.agents/skills/ui-ux-pro-max/) | Pinned UI/UX Pro Max v2.14.1 skill snapshot, MIT license, provenance/hashes, CSV data, search engine, and reference rules. |
| [src/lib/mockups/brand-directions.ts](src/lib/mockups/brand-directions.ts) | Prior deterministic brand-kit triads retained as the whole-run fallback and Pro Max rollback path; `MOCKUP_BRAND_DIRECTIONS_ENABLED` remains the master switch. |
| [src/lib/mockups/brand-directions-bank.generated.ts](src/lib/mockups/brand-directions-bank.generated.ts) | Generated 15-kit brand bank; regenerate with `node scripts/build-mockup-brand-bank.mjs`, never hand-edit. |
...
| [src/lib/mockups/option-drafts.ts](src/lib/mockups/option-drafts.ts) | Server helper for durable partial mockup option and authoritative design-plan persistence, cross-row consistency checks, recovery normalization, authorization checks, and cleanup. |
