...
| [src/app/(dashboard)/projects/page.tsx](src/app/(dashboard)/projects/page.tsx) | Projects dashboard — loads owned projects, allowance status, and ordered A/B/C mockup preview metadata, then renders cards with carousel controls, project action menus, and paid-plan delete gating |
| [src/components/projects/dashboard-project-card.tsx](src/components/projects/dashboard-project-card.tsx) | Interactive project card with hover/touch-aware vertical kebab, arrows, accessible position dots, swipe-safe workspace link, workspace warmup, Rename/Export/Delete actions, export toast state, dialogs, and Free-plan upgrade prompt |
| [src/components/projects/dashboard-project-card-details.tsx](src/components/projects/dashboard-project-card-details.tsx) | Figma-matched 122px transparent details area with a single-line title/date row and natural 72px hard-clipped description flow |
| [src/components/projects/dashboard-project-thumbnail.tsx](src/components/projects/dashboard-project-thumbnail.tsx) | 378px rounded, bordered white media surface with a 336px canvas, lazy authenticated A/B/C images, per-slide loading spinner, cached-image detection, horizontal swipe detection, and distinct missing/failed-image and query-unavailable states |
...
| [src/components/landing/hero-dot-field.tsx](src/components/landing/hero-dot-field.tsx) | Reusable ambient section-background canvas for landing sections: deterministic clustered dot lattice, cursor-proximity dot lift, optional compass wedges, event-driven animation parking, lifecycle gating, and pointer-exit reset |
...
| [src/components/ui/report-table.tsx](src/components/ui/report-table.tsx) | Shared Figma-aligned HTML table shell for project reports, including responsive overflow, header/zebra styling, and optional body-cell emphasis |
