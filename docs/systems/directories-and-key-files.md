...
| [src/components/landing/hero-dot-field.tsx](src/components/landing/hero-dot-field.tsx) | Ambient section-background canvas: deterministic clustered dot lattice mounted inside each selected landing section, so native document/sticky layout moves or pins it with its content; cursor-proximity dot lift, sparse cursor-pointing compass wedges, event-driven animation parking, lifecycle gating, and pointer-exit reset |
...
| [src/lib/mockups/brand-directions.ts](src/lib/mockups/brand-directions.ts) | Deterministic per-project brand kit triads (platform-aware archetypes, OKLCH palettes, semantic ramp, anti-slop rules) injected into mockup image prompts; `MOCKUP_BRAND_DIRECTIONS_ENABLED` kill switch. |
