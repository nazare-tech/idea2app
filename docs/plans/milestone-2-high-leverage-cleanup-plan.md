...
5. [ ] Workspace decomposition per Clarifying Question 1, one hook per commit:
   ...
   - Manual QA after each commit using the relevant `docs/plans/qa-test-plan.md` flows (load workspace, generate a document, refresh mid-generation, hash navigation, version switching).
...
7. [ ] Monitoring per Clarifying Question 3:
   ...
   - Add an alert for repeated Stripe webhook failures (closes `docs/plans/security-release-checklist.md` section 3 items).
...
9. [ ] Rewrite `README.md`: what Maker Compass is, prerequisites, env var table (names and purpose only, no values), `npm run dev/test/lint/typecheck`, pointer to `PROJECT_CONTEXT.md`, `docs/`, and `docs/plans/`.
...
- Manual QA flows from `docs/plans/qa-test-plan.md` after each decomposition commit
