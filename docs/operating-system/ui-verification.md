...
- **Standalone-package exception:** when the feature explicitly requires a separately deployable server, the owning thread may keep its existing Maker server and start one additional loopback-only package server on the package's documented port. Record both ownership and the reason in the review artifact; never replace or kill the other server.
