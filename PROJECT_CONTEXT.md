...
- **AI Prompts Workspace Section**: A derived workspace section rendered after Design Mockups. It does not create a database table, generation queue item, or PDF export type. It renders a Recommended AI Build Tool card above Next Prompt, followed by AI Build Guardrails, AI-Friendly Build Sequence, Functional Requirements, and User Stories & Acceptance Criteria with their own `ai-prompts-*` anchors. Post-submit onboarding and manual Generate All progress UIs may show AI Prompts as a display-only derived row once the Product Plan and First Version Plan handoff content is ready, but it is never executed as its own queue item.
...
- New competitive-analysis rows include metadata:
  ...
  - `prompt_version: "competitive-analysis-v2-2026-07-05-evidence-limited-competitors"`
