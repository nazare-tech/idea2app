...
- **Idea Intake Wizard**: Canonical new-project flow at `/projects/new`. Users enter an idea, answer 4-7 AI-generated structured questions, then the app creates the project and starts the bundled onboarding document-generation queue. The intake prompt may add one or two concise tool-fit questions when those answers materially improve the final AI build-tool recommendation. The question parser always ensures a required single-select `primary-platform` question with stable canonical option IDs (`desktop-web`, `mobile-web`, `native-mobile-app`, `native-desktop-app`) and user-facing labels Desktop website, Mobile website, iOS / Android app, and Mac / Windows app; project creation validates that this question has exactly one supported answer so stale/bypassed clients cannot omit it. The wizard stores readable summaries in `projects.description`, structured intake JSON in `project_intakes`, and shows the Maker Compass loading state until Overview + Market Research are ready.
...
- **Product Plan Generation**: Complete Product Plans with exactly three user personas, user stories, grouped requirements, and release planning. Production Product Plan generation and Prompt Lab defaults share the same Product Plan request builder for system prompt, user prompt shape, default model, max tokens, and temperature. Product Plan generation passes the full latest Market Research document into the shared secure prompt builder rather than applying the older production-only 6,000-character downstream trim; the secure prompt builder still applies its normal per-field safety limits. Completed Product Plans render in the workspace as structured Pencil-style blocks through a parser/view-model layer, with markdown fallback for legacy or malformed content. The parser accepts both the older Product Plan H2/H3 contract and the current numbered PRD-style prompt sections for introduction/overview, goals, team and milestones, success metrics, personas, user stories and acceptance criteria, functional requirements, technical considerations, out-of-scope items, risks, dependencies, assumptions, and open questions. Current numbered PRD-style documents are orientation-first: Introduction/overview explains project size, suggested team shape, and first milestone; Goals, Team and Milestones, and Success Metrics render before dense personas and implementation detail. Functional Requirements and User Stories & Acceptance Criteria remain generated in Product Plan markdown but are visually moved into the derived AI Prompts workspace section after Design Mockups. Legacy Product Plan documents continue to use the specialized legacy block layout with persona/profile grouping, compact labeled rows, vertical requirement categories, cleaned horizontal rules, and user-story acceptance criteria cards.
- **First Version Plan Generation**: Strategic first-release development plan based on the Product Plan. Production First Version Plan generation and Prompt Lab defaults share the same First Version Plan request builder for system prompt, user prompt shape, default model, max tokens, and temperature. First Version Plan generation passes the full latest Product Plan document into the shared secure prompt builder rather than applying the older production-only 6,000-character downstream trim; the secure prompt builder still applies its normal per-field safety limits. Completed First Version Plans render in the workspace as structured Pencil-style blocks through a parser/view-model layer, with markdown fallback for legacy or malformed content. The parser accepts both the older First Version/MVP H2/H3 contract and the current 10-section risk-first First Version Plan contract for summary, key risks/assumptions/scope decisions, target user/problem, MVP goal and definition of done, consolidated core user flows, suggested build approach with tactical shortcuts for speed to market, recommended AI build tool, AI-friendly build sequence, validation plan, and next prompt. Current numbered First Version Plan documents use a purpose-built visual block layout that surfaces risks and concrete validation actions before build detail, consolidates core flow/scope/features into one readable table, and separates validation audience, recruiting path, one combined research-plan table, and phase exit thresholds. Recommended AI Build Tool, AI-Friendly Build Sequence, and Next Prompt remain generated in First Version Plan markdown but are visually moved into the derived AI Prompts workspace section after Design Mockups. Direct H2/H3 content is preserved as fallback section cards when a generated First Version Plan lacks deeper nested headings.
- **Mockup Generation**: Three UI mockup alternatives generated from First Version Plans. The default path uses OpenRouter image-output model `openai/gpt-5.4-image-2`, configurable with `OPENROUTER_MOCKUP_IMAGE_MODEL`; images are stored in private Supabase Storage and rendered through an authenticated image proxy. Before image generation, the pipeline creates a compact deterministic mockup brief from the project name, selected primary platform, extracted target user, MVP workflow, core capabilities, candidate screens/features, and UI constraints. The hidden planner receives that compact brief instead of whole Product Plan / First Version Plan documents, then creates a hidden `mockup-design-plan-v1` JSON plan using `OPENROUTER_MOCKUP_PLANNER_MODEL`, falling back to `OPENROUTER_ANALYSIS_MODEL` and then `openai/gpt-5.4-mini`. That plan chooses the primary platform, copies/condenses the input-derived `targetUser` instead of inventing a new persona, defines a populated happy-path scenario, selects exactly two platform-specific core MVP screens, captions, and exactly three complete visual directions labeled A/B/C because every storyboard skeleton has two fixed frames. Over-limit planner output is normalized after the final effective platform is known so storyboards cannot silently retain extra compressed frames. Invalid planner output gets one stricter JSON-only retry and then fails clearly rather than falling back to generic directions. The final image prompt is built from the validated hidden plan and static rendering/composition rules only; raw MVP/Product Plan/persona context is not re-sent to the image model. The hidden plan is stored in `mockups.metadata.design_plan`, not in the user-rendered mockup content; prompt version and prompt character counts are stored in mockup metadata for regression diagnosis. Each option image is now a v2 storyboard (`openrouter-image-v2`): one wide horizontal image containing the selected screens, with display-safe screen names/captions stored in content metadata. OpenRouter image calls now send skeleton-derived `image_config.aspect_ratio` values by default (`21:9` for desktop skeletons, `4:3` for mobile skeletons), while explicit `OPENROUTER_MOCKUP_IMAGE_ASPECT_RATIO` and `OPENROUTER_MOCKUP_IMAGE_SIZE` env values still override or extend provider config. Image-call text output is capped by `OPENROUTER_MOCKUP_IMAGE_MAX_TOKENS` (default `4096`) while the hidden planner call is capped by `OPENROUTER_MOCKUP_PLANNER_MAX_TOKENS` (default `2048`) to keep retries within normal key limits. Actual decoded image dimensions are recorded when available, and generated images with decoded dimensions outside the expected skeleton aspect range fail before upload/finalization so compressed square desktop storyboards are not saved. Manual workspace mockup generation still runs separate option-level invocations for Options A/B/C one after another, then finalizes one normal mockup document after all three storyboard images are saved. Manual retries persist the option run id and design plan in localStorage and call `/api/mockups/recover-options` before spending more OpenRouter credits, so images that reached Supabase Storage can be finalized even if the browser or route timed out. Generate All/onboarding mockup generation uses the queue item run id as the Supabase Storage run prefix; while the canonical `mockups` row still finalizes only after all three options are available, each completed option is upserted into `mockup_option_drafts` immediately after upload, the workspace polls `/api/mockups/recover-options` during mockup generation, and manual refresh recovery uses the saved local run id to restore any durable draft options. Draft progress callbacks are best-effort after image upload, so a transient draft-row write failure does not discard already-uploaded images; recovery backfills Storage-only options with insert-only draft writes and then re-reads draft rows so placeholders cannot overwrite or outrank richer live rows. New manual or queue mockup generation opportunistically removes stale abandoned draft rows and unreferenced draft Storage objects for the same project/user, while finalized canonical mockup images remain untouched because saved mockups still reference those paths. Draft storyboard image proxy URLs include `draftRunId`; `/api/mockups/image` allows them only when the authenticated user owns the project, the path matches exact `projectId/runId/option-{a,b,c}-storyboard.{png,jpg,jpeg,webp}` form, and a matching `mockup_option_drafts` row exists. Draft rows are deleted after the canonical three-option `mockups` row is saved; Storage-aware cleanup first skips any path referenced by canonical mockup content or metadata. Local/no-credit fixture testing is available through `/api/mockups/fixture` when running outside production, or in production only with `ENABLE_MOCKUP_FIXTURE_GENERATION=true`. The renderer displays each option as a wide storyboard image plus generated screen captions; final image generation now attaches a platform-specific two-frame skeleton from `public/mockups/skeletons/` and prompts the image model to edit that base image in place, preserving canvas size, device/browser chrome, frame placement, captions, gutters, outlines, and shadows while replacing only the purple placeholder interiors. If upstream context suggests more than two screens, later-flow details are folded into one of the two allowed frames rather than asking the image model to add more frames.
- **AI Prompts Workspace Section**: A derived workspace section rendered after Design Mockups. It does not create a new database table, generation queue item, or PDF export type. The section reads the latest Product Plan and First Version Plan content already loaded in the workspace, then renders a single Recommended AI Build Tool card above Next Prompt, followed by AI Build Guardrails, AI-Friendly Build Sequence, Functional Requirements, and User Stories & Acceptance Criteria with their own `ai-prompts-*` anchors. The recommendation is generated in the First Version Plan from a constrained handoff tool set and includes an external product link, fit rationale, starting-cost note, caution, and handoff instruction.
...
2. **Idea Intake Wizard Flow**:
   ...
   - `POST /api/intake/questions` generates 4-7 AI-authored answer-chip questions with `single` and `multiple` answer modes, including optional tool-fit questions when useful for the AI Prompts recommendation. Standalone text questions are rejected; optional free text is only revealed through `Other` on single-select questions where useful. If AI question generation is unavailable or invalid, the route returns a retryable error instead of falling back to curated questions
...
│ AI Prompts         │ Derived build handoff from Product Plan + First Version Plan
└──────────────┴───────────────────┴──────────────────────────────┘
...
- **`ScrollableContent`** — renders Overview, Market Research, Product Plan, First Version Plan, Design Mockups, and the derived AI Prompts handoff as stacked sections. It defers below-the-fold sections by one animation frame and uses generation placeholders when content is not saved yet.
...
- **Document registry** — document labels, titles, icons, credit cost, nav visibility, Generate All order, and default models come from `src/lib/document-definitions.ts`; scroll-section anchors live in `src/lib/document-sections.ts`, including the derived AI Prompts nav item after Design Mockups.
...
- New competitive-analysis rows include metadata:
  ...
  - `prompt_version: "competitive-analysis-v2-2026-06-22-market-research-no-risks"`
  - `live_research` diagnostics with competitor search status, raw/usable competitor counts, context availability, and Tavily extraction status/counts so missing provider data can be distinguished from true empty research.
...
- **mockup_option_drafts**: Durable partial progress rows for completed storyboard options before canonical mockup finalization
  - Fields: `id`, `project_id`, `user_id`, `run_id`, `option_label`, `option_json`, `model_used`, `source`, `design_plan`, timestamps
  - RLS: Users can only access draft options for their own projects; `(project_id, run_id, option_label)` is unique
...
- **POST /api/mockups/generate**: Generate OpenRouter storyboard mockups
  ...
  - Each uploaded option is upserted to `mockup_option_drafts`; successful canonical save deletes draft rows for that run
...
- **GET /api/mockups/image**: Proxy stored OpenRouter mockup images through the server
  - Query: `projectId`, `path`, optional `mockupId`, optional `draftRunId`
  - Requires auth, verifies the storage path belongs to a saved mockup for a project owned by the current user or to a matching owned `mockup_option_drafts` row, then streams the private Supabase Storage image
...
| [src/components/layout/anchor-nav.tsx](src/components/layout/anchor-nav.tsx) | Sticky/horizontal document rail for Overview, Market Research, Product Plan, First Version Plan, Design Mockups, and derived AI Prompts with queued/generating/ready/needs-retry indicators |
| [src/components/layout/scrollable-content.tsx](src/components/layout/scrollable-content.tsx) | Scrollable document body renderer with deferred sections, queue-aware placeholders, PRD/MVP completed-document block rendering, mockup/status modules, and derived AI Prompts handoff |
...
| [src/lib/document-sections.ts](src/lib/document-sections.ts) | Scroll workspace section registry and anchor IDs for Overview, Market Research, Product Plan, First Version Plan, Design Mockups, and derived AI Prompts |
...
| [src/components/analysis/planning-document-blocks.tsx](src/components/analysis/planning-document-blocks.tsx) | PRD, MVP, and derived AI Prompts block renderers that use the Pencil-style planning document parser/view-model layer with markdown fallback, PRD context/vision cards, compact labeled PRD prose rows, moved requirement/user-story handoff cards, single-profile persona fallback labeling, and MVP overview placement |
...
| [src/app/api/mockups/recover-options/route.ts](src/app/api/mockups/recover-options/route.ts) | POST endpoint to recover durable draft storyboard options for a mockup run before retrying OpenRouter generation, with Storage fallback for older runs. |
