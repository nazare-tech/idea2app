...
**Last Updated**: 2026-05-28 (PromptLab package scaffold)
...
- **PromptLab Package Scaffold**: A package-shaped open-source PromptLab scaffold lives under `promptlab/` as the first step toward a separate package/repository. It exports `promptlab/core` contracts, local JSON persistence, a safe `promptlab init` CLI scaffold, an initial dev-only React modal launcher shell, and a placeholder Next.js integration export. The scaffold is intentionally generic and relies on host-provided project/context/run adapters rather than MakerCompass document-generation APIs.
...
| [promptlab/package.json](promptlab/package.json) | Package-shaped PromptLab scaffold with `promptlab` bin, subpath exports, build/test scripts, and package metadata |
| [promptlab/src/core/index.ts](promptlab/src/core/index.ts) | Generic PromptLab core exports: config validation, context serialization, local JSON persistence, and public types |
| [promptlab/src/core/runtime.ts](promptlab/src/core/runtime.ts) | In-process PromptLab runtime that resolves projects/context, builds prompts, runs artifacts, and persists run records through host adapters |
| [promptlab/src/cli/init.ts](promptlab/src/cli/init.ts) | `promptlab init` file scaffold for safe local config, local JSON persistence, and AI-agent handoff instructions |
| [promptlab/src/react/index.tsx](promptlab/src/react/index.tsx) | Initial dev-only PromptLab modal launcher shell for host React apps |
