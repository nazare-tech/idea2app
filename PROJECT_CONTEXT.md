...
**Last Updated**: 2026-06-22 (AI Prompts derived workspace section)
...
- **Dev Project Animation Lab**: Localhost-only, local-development-only sandbox at `/dev/project-animation-lab` for iterating project creation and document block-reveal animation without backend changes. It is guarded by `isDevOnlyFeatureEnabled()` plus a localhost host check, performs no Supabase/database/API calls, and uses static scrubbed fixture projects from `src/app/dev/project-animation-lab/fixtures.ts`. The client simulation shows the creation loading rows beside a workspace-style preview: Market Research becomes ready first, then Product Plan and First Version Plan reveal completed block chunks after shimmer placeholders, using the production PRD/MVP renderers for completed content where feasible.
...
| [src/app/dev/project-animation-lab/page.tsx](src/app/dev/project-animation-lab/page.tsx) | Localhost-only Project Animation Lab route for static fixture-based project creation and document reveal animation prototyping |
| [src/app/dev/project-animation-lab/project-animation-lab-client.tsx](src/app/dev/project-animation-lab/project-animation-lab-client.tsx) | Client-side animation timeline, controls, loading rows, and workspace-style preview for the Project Animation Lab |
| [src/app/dev/project-animation-lab/fixtures.ts](src/app/dev/project-animation-lab/fixtures.ts) | Static scrubbed fixture projects used by the Project Animation Lab without Supabase or API calls |
