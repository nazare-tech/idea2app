...
**Last Updated**: 2026-05-24 (current prompt planning block layout)
...
- **Product Plan Generation**: Complete Product Plans with user personas, features, and release plans. Completed Product Plans render in the workspace as structured Pencil-style blocks through a parser/view-model layer, with markdown fallback for legacy or malformed content. The parser accepts both the older Product Plan H2/H3 contract and the current numbered PRD-style prompt sections for introduction/overview, goals, personas, user experience, functional requirements, user stories and acceptance criteria, out-of-scope items, technical considerations, success metrics, timeline, risks, dependencies, assumptions, and open questions. Current numbered PRD-style documents use a purpose-built visual block layout that preserves current prompt labels while turning personas into profile cards, key user types into compact tiles, goals/user experience into nested cards or narrative rows, requirements into grouped build-scope sections, and user stories into acceptance-criteria cards. Legacy Product Plan documents continue to use the specialized legacy block layout with persona/profile grouping, compact labeled rows, vertical requirement categories, and user-story acceptance criteria cards.
- **First Version Plan Generation**: Strategic first-release development plan based on the Product Plan. Completed First Version Plans render in the workspace as structured Pencil-style blocks through a parser/view-model layer, with markdown fallback for legacy or malformed content. The parser accepts both the older First Version/MVP H2/H3 contract and the current numbered MVP Plan sections for summary, assumptions, target user/problem, goal, core flow, scope, must-have features, suggested build approach, AI-friendly build sequence, validation plan, cut list, and next prompt. Current numbered MVP Plan documents use a purpose-built visual block layout that preserves current prompt labels while turning the thesis into a dark summary block, target/problem content into nested cards or narrative rows, the core flow into step cards, must-have feature tables into feature cards, and scope/build/validation sections into compact Pencil blocks. Direct H2/H3 content is preserved as fallback section cards when a generated First Version Plan lacks deeper nested headings.
...
- Manual mockup generation can take close to 5 minutes per option because `openai/gpt-5.4-image-2` is generating high-fidelity UI images and each option has a `285s` OpenRouter timeout inside Vercel Hobby's `300s` function limit.
