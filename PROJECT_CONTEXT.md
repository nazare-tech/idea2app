...
6. **Analysis Flow** (individual tab generation):
   ...
   - Routes to the appropriate in-house pipeline (`src/lib/analysis-pipelines.ts`). When a `project_intakes` row exists, generation context is formatted through `formatProjectIntakeForAi()` and combined with the human-readable project summary; otherwise `projects.description` is used as the fallback:
    - **Market Research**: 3-step pipeline — Perplexity (sonar-pro) finds competitors → Tavily extracts URL content → OpenRouter synthesizes final report. Graceful degradation if Perplexity/Tavily fail. External API calls use `withRetry` (3 retries, exponential backoff on 429/5xx). OpenRouter synthesis calls use the shared long-text timeout (`790s`) with clearer timeout messages; direct generation returns safe timeout failures as `504`.
    - **Product Plan**: OpenRouter LLM call with detailed system prompt, receives a bounded market-research context excerpt and can run inside the 800s Pro route envelope
...
7. **Generate All / Onboarding Generation Flow** (server-side, durable):
   ...
   - Store fires `POST /api/generate-all/execute` as fire-and-forget (server runs up to 800s even if user closes tab)
...
- **POST /api/dev/prompt-lab/run**: Run one artifact with editable prompts and model override, then save the isolated result to `prompt_lab_runs`.
  ...
  - Text artifact runs use the shared OpenRouter long-text timeout (`790s`) and return clearer timeout errors instead of the raw "operation was aborted" message.
...
- **POST /api/generate-all/execute**: Server-side pipeline orchestrator
  ...
  - Route `maxDuration`: 800s — durable even if browser tab closes
...
| [src/lib/analysis-pipelines.ts](src/lib/analysis-pipelines.ts) | In-house analysis orchestration (market research, Product Plan, First Version Plan, tech spec). OpenRouter long-form text calls use the shared 790s AbortSignal timeout. |
