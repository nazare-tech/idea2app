...
6. **Analysis Flow** (individual tab generation):
   ...
   - Routes to the appropriate in-house pipeline (`src/lib/analysis-pipelines.ts`). When a `project_intakes` row exists, generation context is formatted through `formatProjectIntakeForAi()` and combined with the human-readable project summary; otherwise `projects.description` is used as the fallback:
    - **Market Research**: OpenRouter-managed Exa first finds competitors and returns citation excerpts → OpenRouter synthesizes the final report. If the Exa request errors, cannot be parsed, has no usable public URLs, or has no citations, the pipeline falls back to Perplexity (sonar-pro) → Tavily extraction before synthesis. External API calls use `withRetry` (3 retries, exponential backoff on 429/5xx); the Exa adapter has a 120s timeout and bounded five-result/five-total search budget. OpenRouter synthesis calls use the shared long-text timeout (`240s`) with clearer timeout messages; Product Plan and First Version Plan use a longer planning-document timeout (`480s`); direct generation returns safe timeout failures as `504`. All pipeline completions check `finish_reason`: a `length` finish means the document was truncated at the model output cap, so the step fails (and the queue can retry) instead of saving a truncated document; the First Version Plan cap is 24,576 tokens because 16,384 truncated real plans mid-table.
