...
- **Project Composer ("Ask this project")** *(feature branch `feature/project-composer-chat`, MVP evaluation spike)*: A floating command-bar chat over the project workspace, implemented from the Project Composer v3 design. Users ask questions about their generated documents; `POST /api/projects/[id]/composer` streams answers from OpenRouter with the OpenRouter `web` plugin enabled for live web search. Scope toggles between the document currently in view (tracked from workspace scroll position) and the whole project; the server re-reads the latest saved Market Research / Product Plan / First Version Plan rows per request. Sessions are ephemeral by design: no database writes, no saved history, no credits, read-only (never edits documents). Rate limited per user (30/hr) and IP (90/hr); user text passes `sanitizeInput` and document context flows through `buildSecurePrompt`. Model comes from `OPENROUTER_COMPOSER_MODEL` (falls back to `OPENROUTER_CHAT_MODEL`, then `anthropic/claude-sonnet-4-6`). UI lives in `src/components/workspace/project-composer.tsx`; prompt in `src/lib/prompts/project-composer.ts`.
