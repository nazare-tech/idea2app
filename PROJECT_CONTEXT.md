...
**Last Updated**: 2026-06-30 (document structure updates)
...
- **AI Prompts Workspace Section**: A derived workspace section rendered after Design Mockups. It does not create a new database table, generation queue item, or PDF export type. The section reads the latest Product Plan and First Version Plan content already loaded in the workspace, then renders Next Prompt, AI-Friendly Build Sequence, Functional Requirements, and User Stories & Acceptance Criteria in that order with their own `ai-prompts-*` anchors.
