...
- [ ] High: Consolidate overlapping effects/state sync in `src/components/workspace/project-workspace.tsx` (localStorage hydration, generation flags, content-length checks) into a simpler coordinated flow.
...
- [ ] High: Lazy-load heavy markdown/code highlighting path in `src/components/ui/markdown-renderer.tsx` (defer `react-syntax-highlighter`, split heavy parsing/rendered sections).
- [ ] High: Throttle/RAF resize drag updates in `src/components/ui/content-editor.tsx` so `documentWidth` is not updated on every `mousemove`.
