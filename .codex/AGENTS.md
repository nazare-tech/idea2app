...
* Include the framework-agnostic Runtime and Change-Impact Analysis required by `/holistic-implementation`. Apply it especially to AI generation, polling/streaming, queues, partial-content persistence, shared state, client-server payloads, caches, and billing-adjacent data. Keep the global skill as the canonical template.
