...
Defines the 7-line greppable header convention, the self-healing docs rule, and the docs/systems/ layout that replaces a single monolithic context file.
...
The self-healing rule: any agent that changes behavior a doc describes must update that doc in the same commit; review personas own doc freshness per system.
PROJECT_CONTEXT.md is an index pointing into docs/systems/ (architecture, database-schema, api-endpoints, coding-conventions, setup-and-build, and more).
...
Related: docs/operating-system/planning-workflow.md, review-personas.md, ui-verification.md; router at AGENTS.md; template at docs/systems/_TEMPLATE.md.
---
...
Exception: files whose name ends in `-template.md` are meant to be copied, so they carry the header shape as placeholder text for the copy to fill, not as facts about themselves.
...
- If a change alters behavior described in a `docs/systems/` doc, update that doc **in the same commit**.
- If a change creates a new system with no doc, create one from `docs/systems/_TEMPLATE.md` and add it to the `PROJECT_CONTEXT.md` index.
...
| Location | Contents |
|---|---|
| `PROJECT_CONTEXT.md` | Index, project one-liner, sources-of-truth table. No system detail lives here. |
| `docs/systems/` | Per-system source-of-truth docs, one per area, from `_TEMPLATE.md`. |
