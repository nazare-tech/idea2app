...
#
# Routes the review to the model that did NOT do the work. Configure reachable
# model IDs below, then confirm them with --dry-run.
#
# Usage:
...
# Reviewer models are intentionally configurable in this portable kit.
CODEX_REVIEW_MODEL="${CODEX_REVIEW_MODEL:-{{CODEX MODEL ID}}}"
CODEX_REVIEW_EFFORT="${CODEX_REVIEW_EFFORT:-medium}"
CLAUDE_REVIEW_MODEL="${CLAUDE_REVIEW_MODEL:-{{CLAUDE MODEL ID}}}"
CLAUDE_REVIEW_THINKING_TOKENS="${CLAUDE_REVIEW_THINKING_TOKENS:-32000}"
...
- Product/user experience: unclear next action, misleading copy/state, accessibility or motion regressions, and friction for the primary user.
- Data/integrity: non-idempotent writes, charge/refund or entitlement asymmetry, stranded/duplicate queues, replay races, migrations, and recovery gaps.
...
if [ "$IMPLEMENTER" = "claude" ]; then
  REVIEWER_DESC="codex exec (${CODEX_REVIEW_MODEL}, reasoning effort ${CODEX_REVIEW_EFFORT}, tools disabled)"
  CMD=(codex exec --sandbox read-only --model "$CODEX_REVIEW_MODEL"
       -c "model_reasoning_effort=${CODEX_REVIEW_EFFORT}" --ignore-user-config --ignore-rules --ephemeral
       --disable shell_tool --disable browser_use --disable browser_use_external
       --disable apps --disable computer_use --disable multi_agent
       --skip-git-repo-check -)
  ENV_PREFIX=()
else
  REVIEWER_DESC="claude -p (${CLAUDE_REVIEW_MODEL}, high thinking, tools disabled)"
  CMD=(claude -p --model "$CLAUDE_REVIEW_MODEL" --safe-mode --setting-sources "" --strict-mcp-config --tools "")
  ENV_PREFIX=(env "MAX_THINKING_TOKENS=${CLAUDE_REVIEW_THINKING_TOKENS}")
fi
