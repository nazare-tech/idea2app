---
name: build-a-brand
description: 'Use when the user asks for build a brand or a task matching the examples below. Build a practical brand identity from an idea, website, reference brands, product photos, or a rebrand request. Default to a fast quick-brand path and offer the full 14-16-page guidelines as an opt-in upgrade. Use for brand identity without a full commerce launch. Trigger phrases include "build me a brand", "brand guidelines for [X]", "create a brand from scratch", "rebrand my [thing]", "visual identity for [thing]", and "build-a-brand".'
...
## Repository Integration Override — Highest Precedence

When the active Git repository contains `brand/README.md`, that repository is the designated brand owner. This section overrides every later instruction that sends final output to `~/Desktop` or an unspecified project working directory:

- Read the existing `brand/brand.md` before starting.
- Keep generated boards, PDFs, assets, tokens, prompts, and zips under `brand/generated/YYYY-MM-DD-<brand-slug>/`.
- Treat `brand/brand.md` as the canonical machine-readable foundation. If it already contains non-placeholder brand decisions, do not overwrite it. Write the proposed replacement to `brand/candidates/YYYY-MM-DD-<brand-slug>.md` and ask before promotion.
- Use collision-safe versioned filenames. Never replace an existing artifact.
- Keep outputs local to the repository unless the user explicitly asks to host or upload them.
- Link the generated package back to `brand/brand.md` so product, UI, copy, and marketing work can find the source of truth.

Repositories without this marker retain the portable upstream output behavior described below.
