...
14. Satisfy the active workspace's pre-implementation review gate, then follow its execution policy:
    - If the workspace requires blocking cross-model plan evaluation, run its documented command against the saved plan before writing code. In this kit: `scripts/agent-review.sh --plan <plan-file> --out <plan-eval-file>`.
    - Triage every finding, update the plan with accepted changes, record rejected findings with reasons, and rerun evaluation after a material plan revision.
    - If the required reviewer is unavailable or fails, mark the plan unevaluated and stop before implementation unless the workspace explicitly defines another policy. Never self-review and call it opposite-model coverage.
    - If the gate passes and the workspace says to proceed autonomously, start implementation.
