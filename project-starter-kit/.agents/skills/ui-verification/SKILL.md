...
## Paid or production actions are opt-in

Routine UI verification must not trigger paid generation, purchases, destructive production mutations, or new external records. Prefer an existing safe state documented by the project. Run a spending or production-changing flow only when the user explicitly requests it in the current thread and the repository policy permits it.
