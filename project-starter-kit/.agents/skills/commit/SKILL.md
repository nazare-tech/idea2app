...
5. Never bypass hooks. `.githooks/pre-commit` runs the project's configured fixer + typecheck on staged code files and refuses partially staged files: stage files whole, and if the hook fails, fix the cause. No `--no-verify` unless the user explicitly asks.
...
2. **Verify.** Run the narrowest reliable checks for the touched surface using the commands documented in `PROJECT_CONTEXT.md` when practical; the pre-commit hook re-enforces its configured checks on code files.
